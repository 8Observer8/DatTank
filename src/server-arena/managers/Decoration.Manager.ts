...
export class DecorationManager {
    ...
    public add ( decoration: DecorationObject ) : void {
    ...
    public init () : void {
        ...
        for ( const decorationType in DecorationManager.structure ) {

            let count = DecorationManager.structure[ decorationType ].count;
            const DecorationType = DecorationsList[ decorationType ];

            while ( count ) {

                const position = new OMath.Vec3( 2350 * ( Math.random() - 0.5 ), 0, 2350 * ( Math.random() - 0.5 ) );
                ...
                const decoration = new DecorationType( this.arena, { position: new OMath.Vec3( position.x, 0, position.z ) });
                this.decorations.push( decoration );
